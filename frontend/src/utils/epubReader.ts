...
/* Finished-book marker (读完页): first open stamps the completion time. */
const DONE_KEY = 'jz-epub-done:v1';
export function loadEpubDone(key: string): number | null {
  try {
    const map = JSON.parse(localStorage.getItem(DONE_KEY) ?? '{}') as Record<string, number>;
    const t = map[key];
...
export function markEpubDone(key: string): number {
  const existing = loadEpubDone(key);
