...
export default function LoginPage() {
  ...
  const caption =
    mood === 'idle' && BEHAVIOR_CAPTION[catBehavior] ? BEHAVIOR_CAPTION[catBehavior]! : MOOD_CAPTION[mood];
  const catProps = {
    mood,
    captchaProgress,
  ...
  return (
    <div className="jz-glass jz-login">
      ...
      <div className={`jz-login-shell jz-fade-in${error ? ' is-error' : ''}`} data-mood={mood}>
        <aside className="jz-login-art" aria-hidden>
          {wide && (
            <div className="jz-login-art-cat">
              <InkCat {...catProps} onBehavior={setCatBehavior} />
            </div>
          )}
          <p className="jz-login-art-caption" key={caption}>
            {caption}
          </p>
          <div className="jz-login-art-foot">一份内容 · 两种形态</div>
        </aside>
