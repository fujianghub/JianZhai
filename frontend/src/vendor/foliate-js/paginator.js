...
const setStylesImportant = (el, styles) => {
    // 简斋 patch: the ResizeObserver can fire `render()` while a freshly
    // created chapter iframe is still parsing (documentElement/body null);
    // the pending `load` re-renders anyway, so just skip instead of throwing.
    if (!el) return
    const { style } = el
