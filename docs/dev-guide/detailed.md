...
| 9 | AI 助手 | 多供应商路由 / 8 操作 + 自定义模板 / 多轮对话 / 日预算与降级 / 用量审计与热图 |
| 10 | 全文搜索 | jieba + tsvector 怎么联动 |
| 11 | 导出 | 5 格式 + anthology 单文件壳 + 安全下载 |
| 12 | 视觉系统与题记 | 4 主题 + 图标体系（三区三语言）+ 题记轮播 + 印章 favicon + PWA |
| 13 | 多用户与安全 | 根管理员分级 / scope_queryset / 友邻闸门 / CSRF / HTTPS dev |
| 14 | 部署与运维 | 完整启动命令 + LAN HTTPS 模式 + 腾讯云生产套件 `infra/` |
...
| `ai` | `services.py` `prompts.py` `pricing.py` `views.py` | **多供应商**代理（Claude + 通义千问）、降级链、日预算、自定义模板、多轮对话、用量日志、价格估算 |
...
每次 `run_once` / `run_stream` / `run_chat_stream` 写一行；用于 `/admin/ai` 用量 Tab 的日历热图与按模型/操作/KB/文档聚合 + CSV 导出。

另有：**`AIPromptTemplate`**（用户自定义 AI 操作：owner / name / icon / instruction / requires_selection / replace_mode，操作 id 形如 `tpl_<id>`）、**`AIConversation`**（多轮对话历史：owner / title / `messages` JSON / model / document）。

### HeroSettings（题记，单例）

```python
class HeroSettings(models.Model):
    """apps/accounts/models.py — 博客首页题记轮播。"""
    ...
    rotation_seconds = PositiveIntegerField()
    enabled = BooleanField(default=True)
```

端点：`GET /public/hero/`（匿名精简）、`GET|PATCH /auth/hero/`（员工）、`POST /auth/hero/batch/`（批量导入，`replace`/`append`；强分隔 `—`/`-` 优先于 `·`，行首 `〔朝代〕` 等括号识别为朝代）。前端 `/admin/hero` 管理页：dnd-kit 整行拖拽排序、预览翻看、「导出」反向生成批量导入文本（`quotesToBatchText`）。
...
/api/v1/ai/templates/  /templates/{id}/      自定义操作模板 CRUD
...
```
浏览器 ──POST /api/v1/ai/stream|chat/──► Django apps.ai ──路由 provider──► Anthropic SDK (Claude Opus 4.7 / Sonnet 4.6 / Haiku 4.5)
       │                                      └──────────────────────────► DashScope OpenAI 兼容端点 (通义千问 Max / Plus / Turbo / VL-Max / VL-Plus)
       ◄──── SSE: data:{"delta":"..."} ──
```
...
- 后端校验 `AVAILABLE_MODELS` 白名单（`apps/ai/services.py`），每项带 `provider`（`anthropic`/`qwen`）、`vision`、`thinking` 能力位
...
此外用户可建**自定义模板**（`AIPromptTemplate`：name / icon / instruction / requires_selection / replace_mode），操作 id 形如 `tpl_<id>`，并入 `/ai/capabilities` 与各 AI 菜单。

### 9.2.1 多轮对话 / 视觉 / 扩展思考

- **多轮对话**：`POST /ai/chat/`（SSE）+ `AIConversation` 持久化 messages JSON，UI 上限 50 轮，对话历史可在 `/admin/ai` 查看/删除
- **视觉输入**：请求带 `images`（`data:image/*;base64`），传给 Claude 或 Qwen-VL（`vision=True` 的模型）
- **扩展思考**：`AISettings.enable_thinking=True` 且模型支持时，分配 `max_tokens//2` 思考预算（Claude 4）
...
另支持**按 KB / 文档归因**（`AIUsageLog.document` / `knowledge_base`）与 **CSV 导出**（`/ai/usage/csv/`）；`/ai/estimate/` 可在真正调用前预览 token / 花费。
...
`apps/ai/pricing.py` —— Claude 单位 USD/MTok；Qwen 原价 CNY/MTok，经 `CNY_TO_USD` 换算后统一以 USD 估算：

| 模型 | input | output | 备注 |
|------|------:|------:|------|
| claude-opus-4-7 | $15.0 | $75.0 | 默认 / 最强推理 |
| claude-sonnet-4-6 | $3.0 | $15.0 | 平衡 |
| claude-haiku-4-5 | $1.0 | $5.0 | 最快 |
| qwen-max | ¥20.0 | ¥60.0 | 阿里顶配文本 |
| qwen-plus | ¥4.0 | ¥12.0 | 性价比 |
| qwen-turbo | ¥1.5 | ¥6.0 | 速度优先 |
| qwen-vl-max | ¥20.0 | ¥60.0 | 视觉旗舰 |
| qwen-vl-plus | ¥8.0 | ¥24.0 | 视觉平价 |

更新价格只改这一个文件；热图与日预算估算共用同一张表。

### 9.5 限流、日预算与失败降级

- `AIWriteThrottle` (`UserRateThrottle scope=ai_write`)：30/min/user
- **日预算**：`AISettings.daily_budget_usd_per_user`（0 = 不限）；`check_daily_budget(user)` 在调用前按当日 `AIUsageLog` 估算花费，超额返回 **429**
- **失败降级**：`fallback_enabled=True` 时，**首 token 前**异常按 `FALLBACK_CHAIN` 自动换模型重试（Opus→Sonnet→Haiku、Max→Plus→Turbo、VL-Max→VL-Plus），成功后在日志记 `fallback_from`
- 供应商未配 Key：该供应商的模型不可选（capabilities 标注），全部未配置时 AI 端点优雅降级
- Admin 主开关关闭：所有 `/ai/*` 返回 503
...
- 数据：单例 `HeroSettings`（见 §3）；`GET /public/hero/` 匿名读
- 渲染：古风单行三色 `〔朝代〕作者〈篇名〉` + 「」角标 + 卷尾金线；4 种动画 fade / slide / typewriter / ink-wash（两段式 enter/leave class 过渡）
- **随机播放**（v0.9.10）：`utils/heroPlayback.ts → buildPlayOrder`（Fisher-Yates）——random 模式每次开页重新洗牌、整轮不重复；sequential 按列表顺序
- 交互：**悬停暂停**轮播、**点击切下一条**（`.jz-hero-rotator-shell`）
- 管理页 `/admin/hero`（员工）：dnd-kit 整行拖拽排序（把手列 ⠿）、预览卡 ‹ › 翻看、批量导入（`replace`/`append`）、「导出」Modal 反向生成批量导入文本（复制 / 下载 .txt）、宣纸纹理预览框
...
匿名 / 普通用户 → 空集；作者（`is_staff`，admin + root）→ **不过滤、共享单一内容池**。
**不再按 owner 隔离作者之间**；`field` 参数为兼容旧调用点保留，已不参与过滤。
个人性数据（AI 对话/模板、收藏、个人资料）仍按 user 隔离；读者的收藏/评论**故意绕过** scope，按博客可见性取公开文档。

### 13.1.1 四角色权限（v1.0 RBAC）

`apps/accounts/permissions.py` 唯一权威 `get_role(user) → "anon" | "user" | "admin" | "root"`：

- **匿名 anon**：未登录，仅公开博客（受友邻闸门）
- **普通用户 user（读者）**：登录、`is_staff=False`，只浏览博客 + 收藏 + 评论 + 改自己资料，**无创作权**
- **管理员 admin（作者）**：`is_staff=True` 非 superuser，建改 KB/文件夹/文档、上传/导入、AI、导出、题记、用户管理；可软删文档/文件夹
- **根 root**：`is_superuser` 且 username == `ROOT_ADMIN_USERNAME`（环境变量，非 DB 字段），全局唯一、不可被禁用/删除

要点：

- **删除分级**：软删文档/文件夹 = 作者（`IsContentAuthor`）；**删 KB / 删大类 / 永久删 / 清空回收站 = 仅根（`IsRoot`）**
- **权限类**：`IsContentAuthor`（=`is_staff`）/ `IsRoot`（=`is_root_admin`）/ `PublicOrLoginGated`（**友邻闸门**：`SITE_REQUIRE_LOGIN=true` 时 `/public/*` 需登录，`session` 端点回传 `require_login`）
- **用户管理**：root 见全部、admin 仅见普通用户 + 自己；**建/提拔管理员（写 `is_staff`）= 仅根**；`can_manage_user(actor, target)` 统一裁决；根账号只能本人改
- 新建账号**邮箱必填**；**账号自服务**（校验当前密码）：`POST /auth/me/change-password|change-email|change-username/` + `/auth/me/avatar/`
- 后端是唯一安全边界；权威清单见 [`../permissions.md`](../permissions.md)
...
`SITE_REQUIRE_LOGIN=true` 开启**友邻可见**；`JIANZHAI_ROOT_ADMIN_USERNAME` 指定根管理员。域名 / ICP 备案 / DNS 指南见 `infra/README.md`。
...
| **新 JzIcon** | `frontend/src/components/common/JzIcon.tsx` 加图标函数，记得给彩点；侧栏图标走 `JzIconKit.tsx`（设计稿系列 + tone 色） |
| **新题记动画** | `styles/`（hero 轮播 CSS 动画）+ `HeroSettings.animation` 选项 + `/admin/hero` 预览 |
...
JIANZHAI_ROOT_ADMIN_USERNAME=yourname       # 根管理员账号（不可被禁用/删除）
