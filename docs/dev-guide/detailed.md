...
### 13.1.1 权限分级（v0.9.9 根管理员）

`apps/accounts/permissions.py`：

- **根管理员** `is_root_admin(user)` = `is_superuser` 且 username == `ROOT_ADMIN_USERNAME`（环境变量 `JIANZHAI_ROOT_ADMIN_USERNAME`，非 DB 字段）；只有根可禁用/降权/重置**其他超管**，根本身不可被禁用/删除
- `can_manage_user(actor, target)` 统一裁决：非根员工不能动超管/彼此；禁止自我降权/自禁用
- 权限类：`IsStaffUser` / `IsSuperUser` / `PublicOrLoginGated`（**友邻闸门**：`SITE_REQUIRE_LOGIN=true` 时全部 `/public/*` 需登录，`session` 端点回传 `require_login` 供前端引导）
- 新建账号**邮箱必填**（`UserSerializer.email` 在 create 时 `required=True`）
- **账号自服务**（需校验当前密码）：`POST /auth/me/change-password|change-email|change-username/` + `/auth/me/avatar/`；前端 `/admin/profile`
