...
| `editor` | `views.py` `tasks.py` `media_auth.py` `media_views.py` `services/{docx_import,epub_sanitize,slides,derived,text_extract,ocr,posters,media_gc}.py` | 附件上传（按类型上限 `max_upload_size_for`）、Word / MD / ZIP / PPT / PDF / EPUB 导入（docx 走 mammoth、epub 上传即净化）、本地图片打包；**派生文件 `DerivedFile`**（PPT 中间 PDF / OCR 副本 / 海报 / 封面）、`ConversionJob` 转换可观测、`DocumentExtract` 附件文本进搜索；**`/media/*` 字节层鉴权 `media_access_status`**（生产 Caddy `forward_auth`，dev `serve_media` 内联 + Range 206）；永久删除经 `media_gc` 清盘 |
