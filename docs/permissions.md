...
| 划线 / 笔记 / 书签（`apps.reading`，2026-09-02；EPUB 用 CFI 锚、Markdown 阅读页用 TextQuote `selector` 锚、PDF 用 `{kind:'pdf', page, quads}` 锚，同表三选一；`ReadingPosition` 服务端续读同规则）——**私人**：只读写本人行，作者也看不到读者的划线 | ✅ | ✅ | ✅ |
