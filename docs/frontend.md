...
- **按钮体系（2026-09-03 收编）**：**`common/IconButton`** 是全站唯一图标钮（AntD text 钮 + 稳定类 `.jz-icon-btn`；`size` xs 22 / sm 26 / md 30 / lg 36 方形热区、`tone` accent/gold/danger、`active` 压下态 `aria-pressed`+`.is-on`、`tooltip` 可选），四态全走 tokens.css 新令牌（`--jz-radius-xs…pill`、`--jz-hover-bg`/`--jz-hover-accent-bg`/`--jz-active-bg`/`--jz-focus-ring`），后台 `.jz-admin-glass .ant-btn` 的 `!important` 刷白对它豁免（`:not(.jz-icon-btn)`）；`iconButtonDiscipline.test` 禁新裸 `<Button type="text" icon={…} />`（编辑器 `.jz-toolbar-icon-btn` 是既有成熟范式豁免）。**披露与方向分工（2026-09-03 第四轮，用户拍板）**：`common/Disclosure` = 圆角实心小三角（macOS 披露三角 / 有道云 / Notion toggle 语汇，`stroke-linejoin:round` 同色描边做圆角，弹簧旋转 90°、hover 放大 1.12），承担一切「下方还有内容」——博客目录树 / EPUB 目录 / KB 页折叠标题 / 代码块折叠 / 后台 AntD Tree switcher / 全局 Collapse expandIcon / 编辑器 details 与标题折叠伪元素（同一路径经 `mask`）；`common/Chevron`（圆头描边尖角号）只做方向/导航。**2026-09-09 真正接线**：此前 `Chevron.tsx` 零使用点、全站方向尖角号实为 AntD 发丝线（`Left/Right/Up/DownOutlined` 8+5+6 处、`CaretDownOutlined` 1、`ArrowUp/DownOutlined` 2、字符 `›` 1）；现 `Chevron` = Lucide 比例 **2.0 圆头**（比 1.5 线稿高一档，标记要在 12–14px 旁 CJK 站住）+ `direction` 四向 + `double` 双尖角，三阅读器翻页/上下章、上下篇、题记预览、六个下拉 caret（`actionIcons.CaretIcon` 现即 `Chevron down 12px`）、代码块子菜单、搜索/查找上下处、滑块验证码把手全走它；AntD Select 内建箭头与 Dropdown 子菜单箭头经 tokens.css `--jz-chevron-mask` 换同一字形（svg 隐藏、`::before` mask，open 态转 -90°）；innerHTML 面 `actionIconSvg('chevron-right' | 'caret')` 同几何。`chevronDiscipline.test` 禁止再 import AntD 方向图标（带尾杆的 ArrowLeft/Right 是箭头语义保留）并锁三处路径与 mask 令牌同源。**Disclosure 统一 11px 实墨色 0.85**（`--jz-disclosure-size`，此前 Tree 9 / Collapse 10 / KB 折叠 11 muted 0.7，与 4px 叶子圆点权重接近读不出层级），tiptap.css 两处 mask 改引 `--jz-disclosure-mask`，`ReadGrantControl` 的 Tree 补 `switcherIcon`。树行叶子位 3–4px 圆点、活动行亮 accent。全展/全折用 `JzExpandAllIcon`/`JzCollapseAllIcon`（三行 + 披露三角），替换 AntD 方框加减：博客目录树 / EPUB 目录 / KB 页折叠标题 / 代码块折叠 / 后台 AntD Tree `switcherIcon` / 全局 `ConfigProvider.collapse.expandIcon` 都渲染它（AntD Tree 自带的 svg -90° 已 `!important` 中和），编辑器标题折叠伪元素改单字形 `▸` 旋转。**收藏/置顶** `common/DocPinFavoriteButtons`（泛型 doc；图标是自绘双色 `JzStarIcon`/`JzPinIcon`。**2026-09-09 重定色 + 换钉**：静息一律回到 Jz 图标默认语法（`--jz-text-muted` 1.5 描边 + `--jz-icon-fill` accent 14% 浅填），删除 theme.css 把静息星刷成金混墨、静息钉刷成墨的覆盖——`--jz-gold` 在星空/深海/寒梅三主题是紫/青/梅红，金混墨静息态在那三套里是一整列彩星；点亮才借主题色（星实心 `--jz-gold` + `jz-star-pop`，钉实心 `--jz-accent`）。图钉改 **Tabler `pin` 斜钉**（正视钉 13px 下像烧瓶、靠 CSS -35° 才有语义），斜向自带「钉住」，删 CSS 旋转；星芒删除（15px 下约 1.4px 不可见）。历史：，用户否决了 AntD 星/钉「丑、不达意」，第二轮再改圆角相接 1.75 描边几何（Tabler 式）与圆形热区、弹簧 hover（星缩放 -12° / 钉 -18° 抬起）；**默认常驻显示**兼作状态徽标，`reveal` hover 显现改为可选项；树行/后台树的动作区从标题前移到**行尾（格式标签之后）**，静态 0.78 不透明、行 hover 满亮；星标点亮 `jz-star-pop` 弹簧 + 金色光晕）四处共用：博客目录树、后台 KB 树（手抄副本已删）、**KB 页行/卡片（新）**、**文章页工具条（新，详情序列化器补 `is_pinned/is_favorited`）**；收藏成功由各处 handler 触发 `burstAtPointer` 墨点。
...
- **文字层 + 链接层 + 精确跳转（2026-09-08）**：每页从纯 canvas 改为 pdf.js 三层栈 `.jz-pdf-page-inner > canvas + .textLayer.jz-pdf-textlayer + .jz-pdf-linklayer`（`components/common/pdf/renderPdfPageLayers.ts`，PdfCanvas 与 PPT 单页视图共用）。**文字层**用 `pdfjs.TextLayer`（viewport 传 **CSS scale 不乘 dpr**，pdf.js 内部自乘 pixelRatio 量字形），完成后追加 `.endOfContent` 并交给 `utils/pdfTextSelection.ts`——复刻 `TextLayerBuilder#bindMouse` + 全局 `selectionchange` 管理器（`.selecting` 撑满 endOfContent 让拖选跨越空白/跨页；`copy` 事件 `normalizeUnicode` 去 NUL）；**不整体 import `pdf_viewer.css`**（1000+ 行 `.pdfViewer` 壳 + 图片资源），只把 `.textLayer` 规则块搬进 `reader.css` 限定在 `.jz-pdf-textlayer`。**尺寸契约**：inner 内联 `--scale-factor`（cssScale）与 `--user-unit`（`page.userUnit`），CSS 派生 `--total-scale-factor` 并**必须**给 `--scale-round-x/y:1px`——pdf.js 5.7 `setLayerDimensions` 在支持 `round()` 的浏览器写 `round(down, …, var(--scale-round-x))`，缺变量宽高整体无效、文字层塌成 0 宽且无报错。**链接层**：`page.getAnnotations` 过滤 `/Link`，`utils/pdfLinks.ts` 分类（`url` 仅放行 http(s)/mailto → `<a target=_blank rel=noopener>`；`dest` 内部 GoTo；`FirstPage/LastPage/NextPage/PrevPage` 四个命名动作；JavaScript/Launch 一律忽略），`annotationRectToCss` 经 viewport 归一为 CSS 盒。**精确跳转**：`utils/pdfDest.ts resolveDest` 取 `XYZ`/`FitH`/`FitBH`/`FitR` 的 `top`（PDF 用户空间，`Fit*` 无位置则页顶；explicit dest 首元素允许整数页号），`pdfOutline.ts` 的 `PdfTocEntry` 随之带 `top/left`，`PdfCanvas.jumpToDest` 经目标页 `getViewport(cssScale).convertToViewportPoint` 换算成页内 px 交给 `scrollPageIntoView(n, smooth, innerOffsetPx)`（仍不用 `scrollIntoView`；占位页已按比例排好，目标页未渲染也能落点）。目录点击与正文内链同一条路径。冒烟 `Test/scripts/pdf_text_smoke.py`（用导出器 `_render_pdf` 现造带书签+内链样本，15 项）。注意大纲加载后侧栏自动展开会触发一次整体重绘，冒烟断言前要等层稳定（`settle`）。**未做**：PDF 划线（`Highlight` 锚仍是 cfi XOR selector）、阅读器内查找（浏览器 Ctrl+F 只命中窗口 ±1 屏已渲染页）、翻页键。

- **流式 Range 加载（2026-09-08，批 1）**：`hooks/usePdfDocument.ts` 改为把 URL 交给 pdf.js（参数单源 `utils/pdfLoad.ts buildPdfLoadOptions`：`withCredentials / rangeChunkSize 256KB / disableAutoFetch / disableStream / isEvalSupported:false / enableXfa:false`），每个分片是可缓存的同源 206 GET——67.7 MB 的 989 页文档首屏只取 1.4 MB，翻到中部再按需取；**`disableStream` 必须开**，否则 pdf.js 会让首个整份 GET 在后台一直流到全文件进内存（正是 1.4 GB 扫描件的问题）。旧实现「自己 fetch + 每次挂载 `?_=` nonce」已删（根因是 dev 代理 keep-alive 复用半排空 socket，`vite.config.ts freshSocketAgent` 早已修掉；nonce 只让 Caddy 的 immutable 缓存永远失效），nonce 只在用户点「重试」时追加。加载态显示 `Progress`（`onProgress`，与 EPUB 下载条同款），错误文案 `describePdfLoadError` 中文化（加密/404/损坏/异常状态）。dev 侧 Django `static()` 不支持 Range（永远 200 整份），改挂 `apps/editor/media_views.serve_media`（DEBUG 专用，206/416/Accept-Ranges，经 `media_auth.media_access_allowed` 判定——批 5 填充）。冒烟 `Test/scripts/pdf_range_smoke.py`（206 分片数、首屏传输量 < 30%、无 nonce、按需取片、5× 前进后退与编辑器 inner 模式反复开关无空响应）。**Playwright 同步 API 只在调用 API 时派发网络事件**：`wait_until` 里纯 `sleep` 看不到新响应，条件里要塞一个空 `page.evaluate("1")` 泵事件。

- **交互补齐（2026-09-08，批 7）**：① 键位全部进注册表 `reader.pdf.*`（←/→、PageUp/Down、Home/End、+/−/0、t 目录、b 书签、Mod+F 查找——后两者批 9 接线），PdfCanvas/PptxReader 两处裸 `keydown` 改 `useShortcut`（方向键 `allowInTyping:false`——此前 PptxReader 手写守卫漏掉 contenteditable），新纪律测试 `shortcuts/bareKeydownDiscipline.test.ts`（白名单只剩 EPUB iframe 等有理由的例外）。② 续读：`utils/readerPosition.ts`（PDF `jz-pdf-pos:v1` 存 `{page, offset∈[0,1]}`，PPT `jz-pptx-pos:v1` 存 `{slide}`，key = url 去 query / `post:<id>`），PdfCanvas 首次 settle 后弹内嵌胶囊「继续上次阅读 · 第 N / M 页」（复用 `.jz-resume-pill`，12 s 自动消失，不自动跳），PostDetail 的百分比胶囊对二进制文档关闭；PPT 直接从记忆的 slide 打开。③ 深链：`?page=N`（`PublicAttachmentPreview → PdfCanvas initialPage`，优先于记忆）与 `?slide=N`（1-based），翻页时 `syncUrlParam` 用 `replaceState` 同步（**Playwright `page.url` 不反映 replaceState，断言要读 `location.search`**）；目录项 href 改为真链接 `?page=N`；工具栏「复制当前页链接」。④ a11y：缩放按钮补 aria-label（`iconButtonDiscipline.test` 新增「图标-only `<Button icon>` 必须带 aria-label」），全屏 portal 加 `role=dialog aria-modal` + 进入聚焦/退出归还焦点，`scrollPageIntoView` 尊重 `prefersReducedMotion`。⑤ PPT 工具栏「PDF 版」下载（deck PDF）；编辑器 `FindReplacePanel` 在 pdf/pptx/epub 模式不再打开空壳。冒烟 `Test/scripts/pdf_nav_smoke.py` 22 项。**冒烟坑**：往 body 末尾追加 `<input>` 再 focus 会把页面滚到底、当前页跟着变——探针元素要 `position:fixed`。

- **纸色 / 移动端 / 站点目录接入（2026-09-08，批 8）**：① 纸色单源 `utils/readerPaper.ts`（复用 `EPUB_PAPERS` 五档，key `jz-pdf-paper:v1`，`useReaderPaper` 经 `useSyncExternalStore` 跨标签同步），工具栏 `common/ReaderPaperPicker`（PDF/PPT 共用）：浅色纸传给 `page.render({background})`（pdf.js 自己画底、字与图不动），夜墨是 `.jz-pdf-page-inner[data-paper=night] .jz-pdf-canvas { filter: invert(.92) hue-rotate(180deg) }` 只反色 canvas（文字层/链接层不动、选区色加深；PDF 内图片会一起反色——已知限制）；`PostDetail` 的 `PaperPicker` 对 PDF 仍隐藏（控件=效果契约）。② `PptxReader` 内联样式全部搬进 `reader.css` `.jz-pptx-*`（thumb/主图硬编码 `#fff` 改 `--jz-cell-surface`），`<720px` 缩略轨变横向条、主区独占宽度。③ **阅读页 PDF 目录接入站点目录体系**：`PdfCanvas` 新 props `onOutline/onPageChange/jumpRef`，给了 `onOutline` 即不再渲染自己的侧栏；`PostDetail` 持 `pdfToc/pdfPage/pdfJumpRef`，`canShowToc` 放宽为「PDF 有 outline」，`PostSidePanel` 新 `pdfToc` prop 渲染 `PdfTocPanel`——右栏（≥1281px）/ 抽屉 + FAB（窄屏）/ 目录设置全套一并生效；编辑器/附件面板等 inner 模式仍用阅读器内侧栏，且 `<880px` 时改 AntD `Drawer`（`getContainer={false}`）而非半屏列，全屏也按 `innerWidth` 判断。④ 捏合/双击缩放 `utils/pinchZoom.ts`（纯函数 + `attachPinchZoom`，仅 touch pointer；双击落在文字层上不触发以免与选词冲突），驱动既有 ratio 锚定的 `setZoom`；容器 `touch-action: pan-x pan-y`。冒烟 `Test/scripts/pdf_mobile_smoke.py` 17 项（375px 视口 `has_touch`）。**冒烟坑**：AntD Drawer 关闭后 DOM 仍在，`[role=dialog]` 要带 aria-label 限定；缩放读数用 `data-testid=pdf-zoom` 取（页面另有阅读进度百分比 span）；Home 的平滑滚动未结束就按缩放键，ratio 锚定会按半途位置算——断言前等 `scrollY` 稳定。

- **查找 + 书签（2026-09-08，批 9）**：① 全文查找走**客户端索引** `utils/pdfTextIndex.ts`（首次搜索时逐页 `getTextContent` 拉平成 `{text, items:[{str,x,y,w,h,start}]}`，每 8 页让出 idle，按文档缓存；`searchPdfIndex` 大小写不敏感、每页上限 50 命中，命中带 PDF 用户空间 rect 与 top），原因是窗口化渲染让浏览器 Ctrl+F 静默漏报。命中标记是 `renderPdfPageLayers` 的第四层 `.jz-pdf-hl-layer`（`setMarks` 可对已渲染页即时重画，未渲染页在 render 时带上 `marks`），当前命中 `.is-current` 橙色描边，夜墨下 `mix-blend-mode: screen`。② 面板 `PdfTocPanel` 升级为 Segmented 三 tab「目录 / 搜索 / 书签」，数据经 `utils/pdfReaderApi.ts` 的 `PdfReaderApi` 对象传入（阅读页由 `PdfCanvas onReaderApi` 抬到 `PostDetail` → `PostSidePanel pdfToc.reader`；inner 模式直接传给内部面板），`Mod+F` / 工具栏放大镜 → `onTabRequest('search')`（阅读页按 `useTocDrawer` 只开右栏**或**抽屉，勿两者同开）；搜索 UI 抽成共享 `components/common/reader/SearchResults.tsx`（`.jz-epub-search-*` 类名不变，EpubSidebar 尚未切换、留作后续）。③ 书签：`Bookmark` 加 `page`（reading 0005，`cfi` XOR `page`，两条条件唯一约束，视图按 cfi/page 幂等 get_or_create），PdfCanvas 工具栏书签钮 + `b` 键（`allowInTyping:false`，焦点在搜索框里不触发）+ 书签 tab（复用 EpubSidebar 书签行样式），excerpt 取索引里该页前 100 字。④ PPT：工具栏放大镜 Popover 内同一 `SearchResults`，索引 deck PDF 文本 + 讲者备注，命中跳 slide（`reader.pptx.find`）。冒烟 `Test/scripts/pdf_find_smoke.py` 17 项。

- **卡片海报 / 加密 PDF（2026-09-08，批 10）**：`PublicPost`/`DocumentPreview` 增 `poster_url`/`page_count`（PDF 首页 / EPUB 封面 / PPT 首张缩略图），KB 文章卡（`.jz-post-poster`）、悬浮卡与 doc 卡片嵌入（`.jz-doc-poster`）显示海报与页数，无摘要时显示「N 页」；`usePdfDocument` 接 pdf.js `onPassword`（`PasswordResponses.INCORRECT_PASSWORD` 区分错密），`PdfCanvas` 弹 AntD Modal 输入密码（`getContainer` 指向阅读器容器以备全屏）。

- **划线笔记 + Fullscreen API（2026-09-08，批 11）**：① 第三种锚点 `{kind:'pdf', page, quads}`（PDF 用户空间，同 /QuadPoints 形状；`utils/pdfAnchor.ts`：选区 `getClientRects` 只取**首个命中页**内的矩形 → `mergeLineRects` 同行合并 → `viewport.convertToPdfPoint` 反投影；后端 `HighlightSerializer.validate_selector` 按 `kind=='pdf'` 分派校验，quads ≤200、每条 8 个数、保留两位小数；`Highlight` 类型 `selector` 改联合 `HighlightTextSelector | HighlightPdfSelector`，MdAnnotator 只处理带 `quote` 的）。② 绘制复用批 9 的标记层：`annMarks(page)` 把该页划线转成 `jz-pdf-ann is-<style>` 标记（颜色经 `--jz-ann` 内联，`data-hl` 存 id），与搜索命中合并后 `setMarks`；标记 `pointer-events:none` 以免挡住文字层拖选，**点击命中靠几何**（`onStageClick` 遍历 `.jz-pdf-ann` 的 `getBoundingClientRect`）。③ 交互全部复用 EPUB 件：`EpubSelectionBar`（`selectionchange` 160ms 防抖，锚定相对 `.jz-pdf-stage`）、`EpubHighlightCard`（改色/改样式/笔记/引用/删除）、`EpubNotesExportModal`（`buildNotesMarkdown`，章节名 `第 N 页`），`PdfTocPanel` 加「笔记」tab（登录后），`?hl=<id>` 深链 → `openHighlight`（跳到 quads 顶部 + `is-flash` 1.8 s）。④ **全屏改 Fullscreen API**（PDF/PPT 都从 portal 改为 `wrapRef.requestFullscreen()`，CSS `.jz-pdf-wrap:fullscreen` / `.jz-pptx-wrap:fullscreen`），所有弹层 `getPopupContainer` 指向 wrapper；因此阅读页的 `SelectionAI` 在全屏可用，`contextProvider` 对 PDF 走 `pdfReader.contextText()`（索引已建则取当前页 ±1，否则取已渲染文字层的文本——不主动建索引以免 Range 模式全量下载）。⑤ 明确未做：PPT 划线（`PdfPageView` 侧未接）、划线评论按钮。冒烟 `Test/scripts/pdf_notes_smoke.py` 12 项。**冒烟坑**：阅读页首行文字在 sticky 头栏之下，鼠标/合成选区要选 `top > 160` 的 span；`Range.setStart(firstChild)` 对 pdf.js span 不可靠，用 `selectNodeContents`。
- **服务端阅读位置 + PPT 演示模式 / 翻页动画（2026-09-08，批 12）**：① `reading.ReadingPosition`（reading 0006：`user × document` 唯一，`cfi / fraction / page / offset / slide / updated_at`，每阅读器只写自己懂的锚），`GET/PUT /documents/<id>/position/`（`IsAuthenticated` + `_readable_doc`，PUT upsert、至少一个锚否则 400）。前端 `api/reading.ts getReadingPosition/putReadingPosition`（`keepalive:true` 时改用 `fetch` 以便卸载期写入不被丢弃），`hooks/useServerPosition(documentId, enabled)` 返回 `{remote, loaded, push, waitRemote}`：挂载取一次、`push` 走 `utils/positionSync.createDebouncedPusher`（5 s 合并成一次 PUT，`visibilitychange→hidden` / `pagehide` / 卸载时 keepalive 冲刷），`waitRemote` 给 EPUB 在 `view.init` 前拿服务端位置。**合并规则 `pickNewer(local, remote)`**：localStorage 记录带 `t`、服务端行带 `updated_at`，取新者，**平局与 1 s 内以本地为准**（自己的 push 回声总比本地戳晚几百毫秒，否则同一设备会被当成「从别处续读」）。四阅读器接线：PDF（`{page, offset}`，续读胶囊改按合并结果；**合并决策前不写任何位置**——PdfCanvas 保存 effect 以 `restoredRef` 门控、PptxReader 以 `mergedRef` 门控，否则挂载时的第一次写入会以更新的时间戳把服务端行压掉）、PPT（`{slide}`，无 `?slide=` 深链且用户未翻页时服务端更新者直接打开）、EPUB（`{cfi, fraction}`，relocate 即推）、Markdown（`{fraction}`，`loadReadingPositionRecord` 暴露带戳记录参与合并）。登出用户一切照旧只用本地。② **PPT 演示模式**：工具栏「演示」/ `p`（`reader.pptx.present`）= `requestFullscreen` + `data-presenting`：工具条与缩略轨隐藏、主区独占、右侧 `.jz-pptx-present-side`（计时器 `role=timer` 每秒走、`N / M`、下一页缩略（点击即翻）、当前页备注、键位提示 `<Kbd>`、退出钮），`s`/`b` 黑屏（`.jz-pptx-blackout` 覆盖 wrapper，翻页或点击即恢复），退出全屏即退出演示。③ **翻页动画** `utils/pageTurn.ts runPageTurn({stage, name:'jz-pptx-slide', dir, preload, run})`：`go()` 在 View Transition 里 `flushSync(setActive)`，±1 用滑动、跳转用淡入（复用 EPUB 的 `jz-turn-*` 关键帧，`html[data-jz-turn]{view-transition-name:none}` 根退出快照同款），**先预解码目标图**（`preloadImage` 600 ms 兜底）再开过渡以免新快照空白；`prefersReducedMotion` / 无 VT / 过渡进行中直接切换；`activeRef` 在过渡开始前就更新，连按方向键不丢步。冒烟 `Test/scripts/reading_position_smoke.py` 19 项（PDF 写入/卸载冲刷/新浏览器续读/本地更新者优先、PPT 服务端 slide 打开/VT 计数/演示模式全流程/reduced-motion 无 VT、MD 分数写入与续读）。**坑**：把 `setTimeout` 裸传进对象再 `timers.set()` 调用会以对象为 `this` → 浏览器抛 `Illegal invocation`（Node/vitest 不报），push 抛错把整个阅读器卸载——注入计时器一律箭头包装；卸载期 keepalive `fetch` 拿不到 CSRF，hook 挂载时先 `ensureCsrf()`。
- **扫描件 OCR 接入阅读器（2026-09-08，批 13）**：`PublicPost`/`Document` 增 `reader_pdf_url`（OCR 副本，有则阅读器渲染它、下载仍是原件）与 `ocr_status`（`scanned/queued/running/failed/done/''`），`PublicAttachmentPreview` 与 `DocEditorPage` 的 PDF 分支据此选源并在上方渲染 `PdfScanHint`（`.jz-pdf-scan-hint[data-ocr]`，文字 PDF 与已 OCR 的不显示）。后端细节见 docs/editor.md §8「扫描件 OCR」。
- **PPT 翻页越界 + 「乱码」修复（2026-09-08，批 12/13 后续）**：① 翻页 View Transition 的快照按视口定位，旧页 `translateX(-100%)` 滑出时不受 `.jz-pptx-main` 裁剪、会横穿站点目录栏——`html[data-jz-turn]::view-transition-image-pair(jz-pptx-slide){overflow:clip}` 把新旧快照裁在 stage 盒内（EPUB 的 3D 翻页刻意不加）。② PPT 主区不再让 pdf.js 画 canvas：`PdfPageView visual="image"` 以服务端 `pdftoppm` JPEG 作视觉层（与缩略图同源、字体确定），`renderPdfPageLayers` 加 `layers.raster:false` 只出文字层 + 链接层绝对定位覆盖其上（宿主 `top/left:0`、inner `margin:0` 保证放大后仍逐像素对齐）——pdf.js 渲染 LibreOffice 嵌入子集字体在部分平台字距/字形与 poppler 不一致，用户读作乱码；纸色改由 CSS 给图片 `mix-blend-mode: multiply`（夜墨仍走 invert）。③ 翻页先 `preloadImage`（load + `decode()`）再换 src、主图 `decoding="sync"`，无动画路径也预载——否则换页瞬间主区空白一两帧、VT 的新快照也会截到空白。**探针坑**：Playwright `element.screenshot` 会先把元素滚进视口，翻页动画期间量窗口滚动/截中间帧要用 `page.screenshot`。
...
- **布局**：左侧缩略图导轨 `.jz-pptx-rail` + 中间全分辨率主图 + 工具栏（页码 / 缩放 / 备注 / 全屏）+ 键盘导航（←/→、PageUp/Down、Esc 退全屏）。缩略图轨用 `thumb_url`、主图才拉全分辨率。**2026-09-08 起样式全在 `reader.css` `.jz-pptx-*`**（`data-fullscreen` / `data-paper` 属性驱动变体），窄屏横向轨。
...
- **主区文字层（2026-09-08）**：后端现在保留 LibreOffice 的中间 PDF（`DerivedFile(deck_pdf)`，`slide_pdf_url`），`PptxReader` 经 `usePdfDocument(pdfUrl)` 加载后，主区 `<img>` 换成 `PdfPageView`（单页 `renderPdfPageLayers`：canvas + 可选文字层 + 链接层，宽度 = 主区宽 × zoom，`margin:0 auto` 让超缩放溢出走滚动条而非左侧被裁），**JPEG 留作首绘前的占位**并在无 `pdfUrl`/加载失败时作为最终形态（存量 deck 未回填即自动回退）；缩略图轨、备注、翻页壳不变。slide 内链接：外链新标签；LibreOffice 把**文字超链接**导出为 URI 注解可点，但 python-pptx 的形状级 `click_action.target_slide` 不会导出为 GoTo（冒烟 `pptx_text_smoke.py` 11 项，含无 PDF 回退）。轮询响应带 `slide_pdf_url`，转换完成时一并接上。
...
EPUB 划线的同款体验移植到 `/posts/:slug` 的 Markdown 读路径（HTML 沙箱 iframe 不做；PDF 划线自 2026-09-08 批 11 起以 `{kind:'pdf', page, quads}` 第三种锚点接入 PdfCanvas，见上文「划线笔记 + Fullscreen API」；PPT 未接）。组件 `components/blog/MdAnnotator.tsx`（挂在 `<article>` 内、绝对定位复用 `EpubSelectionBar`/`EpubHighlightCard`/`EpubNotesExportModal`）+ 右栏 `PostSidePanel`（目录 / 笔记 Segmented，`TocPanel` 加 `hideHeader`）+ 纯函数 `utils/textAnchor.ts`（12 单测）。
...
### 目录设置：站点默认 + 本地覆盖（2026-09-03）· 三类集中管理（2026-09-08）

用户拍板：右侧目录个性化 + 默认不换行、后台独立 `/admin/toc` 全局设置、更多字体；2026-09-08 追加：出厂默认改 **紧凑 / 中 / 细 / 宋体 / 淡显**，并按三类目录（大类知识库目录 / 知识库文档目录 / 文档内容目录）在后台集中管理、各自独立默认。

- **三个 scope（2026-09-08）**：`TOC_SCOPES = kblist | kb | article` —— `kblist` = 博客左栏「知识库」列表（`BlogKbNavPanel`，此前是本地专属 `kbToc.KbListPrefs`/`jz-kb-list-prefs:v1`，现并入同一形状并新增 **`grouped`（按大类分组）** 键，CSS 补 `.jz-kb-nav-kbs[data-weight|data-wrap]`）；`kb` = 知识库文档树（`PublicKbFolderTree`）；`article` = 文章标题大纲（`TocPanel` MD/Docx + `PdfTocPanel`）。`TOC_SCOPE_META[scope]`（label / short / hint / 作用范围 chips / `features`）是唯一源，`features` 决定弹层与后台表单显示哪些行（article: depth+numbers；kb: counts；kblist: counts+grouped，且 `colorLayered:false` 隐藏「分层」）。服务端 `TocSettings.prefs` 为 `{scope: prefs}`（`repair_toc_site` / 前端 `repairTocSite` 兼容旧扁平 blob → 摊到三份；迁移 `accounts 0010` 落库转换 + help_text），`GET /public/toc-settings/` 返回 `{prefs:{kblist,kb,article}}`（缓存键 `toc:public:v2`），`PATCH /auth/toc/` 体 `{scope:{subset}}` 可多 scope 同发、扁平体/未知 scope 400，`{reset:true}` 全部、`{reset:'<scope>'}` 单类。`useTocPrefs(scope)` 取 `defaults[scope]` 叠本地覆盖，键仍是 `jz-toc-prefs:v2:<scope>`。
...
- **后端 `TocSettings` 单例**（`apps/accounts/models.py` pk=1 JSONField + `repair_toc_prefs`（单 scope）/`repair_toc_site`（三 scope），`save()` 先修复再清 `toc:public:v2` 缓存、`load()` 内存归一补新键）+ `apps/accounts/toc.py` 两端点：`GET /public/toc-settings/`（`PublicOrLoginGated`，5 分钟缓存）、`GET/PATCH /auth/toc/`（`IsContentAuthor` 读、staff 写，按 scope 严格校验 400、`reset` 全部/单类）。迁移 `accounts 0009`（建表）+ `0010`（扁平→三份）。出厂默认 `DEFAULT_TOC_PREFS`（前后端镜像）= compact / m / light / serif / muted / depth 6 / wrap off / counts·numbers·grouped on。
- **后台 `/admin/toc`**（`pages/admin/TocSettingsPage`，侧栏「目录设置」用 `JzIconKit.JzTocIcon` + 靛蓝 `jz-ico-tone-toc`）：**顶部三张类型卡 `.jz-toc-admin-scope-tab[data-scope]`**（一/二/三序号、状态角标「出厂 / 自定义 / ● 未保存」）切换当前类型，页头 chips 随类型显示作用范围；表单按 `features` 显隐行（排版卡右上「套用到其余两类」把本类草稿复制到另两类，仍需保存），字体画廊 kblist 用「数据中心网络 · 12」字样；右侧预览随类型切换（知识库列表 `KbListPreview` 复用 `.jz-kb-nav-kbs` 真实规则含大类头 / 文档树 / 文章目录）。保存一次 PATCH 送出全部脏 scope（按钮文案「保存并全站生效（N 类）」），「恢复出厂默认」是下拉：仅恢复本类 / 恢复全部三类——**禁用判定看 saved 与 draft 两侧**（草稿被「套用」回出厂值时仍要能重置服务端）。样式块在 theme.css「目录设置页」+「类型切换」。冒烟 `Test/scripts/toc_scopes_smoke.py` 43 项（三类独立默认 / 左栏列表齿轮分组开关 / 后台多类保存·单类恢复·套用 / 旧扁平 blob 兼容）；`ui_batch_smoke.py` 默认值断言同步改 serif/compact。
