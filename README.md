...
| **用户级阅读授权白名单**（2026-07-24） | 用户管理里给普通用户配 `ReadGrant` 授权，四粒度：整 KB（**多选批量**）/ 整大类 / 文件夹（**含全部子文件夹**）/ 单篇文档；**空=不受限（向后兼容），有=白名单，与受众可见性取 AND**；未授权的 KB 名/大类/文件夹/文档在博客端零泄露（含 KB 卡片计数收敛）；staff 不可被授权、软删目标 fail-closed；编辑入口三处（新建用户区块 / 行内按钮 / 「阅读范围」列点击），随时可改、`[]` 即恢复不受限 |
...
| **主题切换电影级过渡** | 过渡指挥器三拍法：默认**分层错峰溶解**（背景先行、玻璃面板/顶栏错峰跟进，旧层失焦退场）、菜单点击**柔边圆形揭幕**（90px 羽化 mask 代替硬边 clip-path）、随朝暮**日落/日出长转场**（1.8s 慢溶解 + 暮色/晨光调色层）；canvas 场景切换改**交叉溶解**（旧场景保活淡出与新场景交叠，不再硬切空一拍）；AntD 主色改运行时读 `--jz-accent` 单源（修复 dark 主题 JS/CSS 色值漂移）；过渡期页面保持可交互。2026-07-25 推 main 并部署上线 |
| **编辑器与文档预览样式优化** | 四路样式深度调研后的第一批落地：富文本**章节编号恢复显示**（折叠箭头伪元素冲突）、表格**列宽拖拽把手**补样式、图片上传状态提示、纸张预设在后台真正生效、打印解除限高（超长表格/代码不再被裁）；暗色下 slash/块面板/AI 菜单等 portal 弹层**不透明化**（新 `--jz-overlay-surface` 令牌）、表格冻结首行/首列**不透明格面**（`--jz-cell-surface`）；编辑态排版逐项对齐阅读端（所见即所得）、内联编辑不再字体跳变；**行内代码**统一派生令牌（阅读/编辑/表格/AI/导出五端一致，六主题对比度实测全过 WCAG AA）；14 类按钮补键盘焦点环 + reduced-motion 兜底。前端 478 测试 + Playwright 六主题实测。2026-07-26 推 main 并部署上线 |
| **语雀空格包裹公式抢救** | 语雀导出的 `$ … $`（内侧留白）撞四端货币防误判边界规则整条退化纯文本；兼容层首步 `rescueSpacePaddedDollarMath`（前后端镜像）仅抢救「独立成行+内侧留白+含 LaTeX 信号」→ `$$` 块，tokenizer 边界规则未动、货币文本不触发；渲染时预处理存量即生效。两端 +11 测试。2026-07-26 推 main，2026-07-27 部署上线 |
...
| **导入体验批次** | 单请求文件数上限调至 1000（Django 默认 100 拒收「文档+图片」整组发送的大文件夹——整组是相对图路径改写的前提）；导入失败/跳过改 notification 列出**文件名+原因**（此前只报个数、明细仅落 console）；上传支持**指定目标文件夹**（导入下拉「上传到」Select + 树上文件夹行「上传到此文件夹」按钮，文件/文件夹/拖拽/带图 MD/ZIP 五入口全生效，目标被删自动回退根目录）。tsc + 前端 518 + 后端 506 测试绿。2026-08-07 推 main 并部署上线 |
| **EPUB 电子书一期：原样阅读器** | vendor foliate-js 的 `EpubReader`：翻页/滚动免重载切换、CFI 位置续读、页顶标题目录高亮、全书搜索、脚注弹卡、六主题 Readium 式注入（字体显式开关）；`.epub` 导入即净化脚本 + 生产 CSP `script-src 'self'`；b 批：目录拖宽/折叠树、出版社框与代码块主题化归一、字体**语义映射**（楷/黑/宋/等宽各归其位）、栏数自动、章节键、图片放大；c 批：代码复制按钮、View Transitions 翻页动画五式、全屏沉浸（弹层入全屏容器）、字体列表与文章页统一 + 思源黑体/本机字体、滚轮翻页、前进后退、剩余时间、目录筛选、CSS Highlight 语法高亮、宽表弹窗、纸色；字体/字号/行距与文章页共用同一偏好、边栏目录重排。Playwright 24+21+32（33MB 真实中文 EPUB）。2026-09-01 |
| **KB 页默认视图调整** | 知识库文档页默认「按文件夹分组 + 列表视图」（密度默认 summary→list）；localStorage 偏好 key 升 v2 使新默认对老访客生效（挂载即自动回写旧默认，仅改代码默认值永不可见）。2026-08-08，已部署 |
| **题记管理升级 + 心境时间戳** | 后台题记列表展示态改造（衬线+三色署名、点击行编辑、顶部草稿式新增、搜索+内滚动）；每条题记新增摘录日期（首页「摘录于」金墨小字，心境时间戳，后台可回填）+ 修改日期（服务端盖章仅后台）；批量导入/导出 `@YYYY-MM-DD` 往返。2026-08-10，已部署 |
