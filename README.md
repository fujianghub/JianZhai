...
| **主题切换电影级过渡** | 过渡指挥器三拍法：默认**分层错峰溶解**（背景先行、玻璃面板/顶栏错峰跟进，旧层失焦退场）、菜单点击**柔边圆形揭幕**（90px 羽化 mask 代替硬边 clip-path）、随朝暮**日落/日出长转场**（1.8s 慢溶解 + 暮色/晨光调色层）；canvas 场景切换改**交叉溶解**（旧场景保活淡出与新场景交叠，不再硬切空一拍）；AntD 主色改运行时读 `--jz-accent` 单源（修复 dark 主题 JS/CSS 色值漂移）；过渡期页面保持可交互。2026-07-25 推 main |
