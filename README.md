...
| **KB 页默认视图调整** | 知识库文档页默认「按文件夹分组 + 列表视图」（密度默认 summary→list）；localStorage 偏好 key 升 v2 使新默认对老访客生效（挂载即自动回写旧默认，仅改代码默认值永不可见）。2026-08-08，未部署 |
| **题记管理升级 + 心境时间戳** | 后台题记列表展示态改造（衬线+三色署名、点击行编辑、顶部草稿式新增、搜索+内滚动）；每条题记新增摘录日期（首页「摘录于」金墨小字，心境时间戳，后台可回填）+ 修改日期（服务端盖章仅后台）；批量导入/导出 `@YYYY-MM-DD` 往返。2026-08-10，未部署 |
| **题记多行支持** | 题记正文支持换行：后台 TextArea 编辑（Enter 换行 / Ctrl+Enter 保存）、首页分行堆叠展示（印章跟末行）、列表 pre-line、批量格式 `\n` 转义往返。2026-08-10，未部署 |
| **字体系统全面升级** | 全站字体自托管（Google Fonts CDN 国内不可达致设计字体从未生效；`@fontsource/*`+文楷+MiSans npm 分片包，唯一入口 `fonts.css`）；八个 `--jz-font-*` 令牌收敛 ~120 处硬编码栈并修行内/块级码不一致等 5 bug；题记/品牌切霞鹜文楷 Screen、后台切 MiSans（`data-scope` 作用域）；阅读器新增「文楷 · 屏显」预设（默认仍宋体）；`--jz-fs-*`/`--jz-lh-*` 排版令牌首批。导出端 PDF 字体一致性为已知限制。2026-08-10，未提交 |
