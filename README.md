...
| **语雀行内代码彩标 + 粘连斜体修复** | 修复颜色块/表格「不支持行内代码」：语雀把染色标签导出在反引号内部，兼容层剥反引号丢代码语义——新增 `convertBacktickedStyledCode` 整段转 `<code>` 芯片（保码丢色、粗斜体保留）；顺带根治相邻斜体粘连 `_A__B_` 的 CJK 侧翼规则怪癖（`normalizeItalicWrappingInlineHtml`，粗体孪生的 `_` 版）。前后端镜像，渲染层预处理存量即生效。2026-09-01 |
