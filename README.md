...
| **导入体验批次** | 单请求文件数上限调至 1000（Django 默认 100 拒收「文档+图片」整组发送的大文件夹——整组是相对图路径改写的前提）；导入失败/跳过改 notification 列出**文件名+原因**（此前只报个数、明细仅落 console）；上传支持**指定目标文件夹**（导入下拉「上传到」Select + 树上文件夹行「上传到此文件夹」按钮，文件/文件夹/拖拽/带图 MD/ZIP 五入口全生效，目标被删自动回退根目录）。tsc + 前端 518 测试绿。2026-08-07，未部署 |
