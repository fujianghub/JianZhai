...
| 文件阅读器 | pdfjs-dist 5（PDF）· 服务端 LibreOffice→JPEG（PPT）· **foliate-js**（EPUB，vendor 固定 commit，`frontend/src/vendor/foliate-js/VENDOR.md`） | EPUB 不选 epub.js（停更）/Readium Web（需 Go 服务端） |
...
- **语雀 MD"识别错乱"= 渲染层三兼容 bug**（存储 `raw_content` 本身干净）：(1) **图表注释被 `-->` 截断（主凶）**——语雀导出 mermaid 为 `<!-- 这是一个文本绘图，源码为：… -->` + 静态 SVG，通用注释剥离在源码内部箭头处截断 → 源码泄漏成正文 + `:::jam` 触发失控 callout；修=`recoverYuqueDiagramComments`（前后端镜像）还原 ```mermaid fence（闭合锚定 `-->`+行尾）并丢静态图，**须在通用剥离前**；(2) `<font>` 交替染色句被强调合并正则吞成整句全粗（连接符须 `[^*\n<]`）；(3) CJK 双加粗被 `**A**B**C**` 合并启发式吞并/静默删标记（该步骤已删,空格+`\w` 防线对中文全失效）；(4) **空格包裹公式**——语雀把公式节点导出为两侧留白的 `$ … $`，撞上货币防误判边界规则整条退化纯文本（可叠加丢反斜杠 `[ … ]` 残骸、`\\frac` 双反斜杠转义）；修=`applyYuqueCompatMode` 首步 `rescueSpacePaddedDollarMath`（前后端镜像）仅抢救「独立成行+内侧留白+含 LaTeX 信号」→ `$$` 块，**勿放宽四端 tokenizer 边界规则**；(5) **URL 内 4+ 连续下划线被拆（四条中唯一会写坏存储的）**——`_{4,}`→`__ __` 相邻加粗拆分对 fence 外全文无差别生效，HillStone 等站 URL 尾部 `TocPath=…_____0` 被插空格 → 链接目标失效退化纯文本，富文本编辑器**每保存一轮把破碎结构写回 `raw_content` 腐蚀一层**（`\[` 转义+部分 autolink+`%5C_`，第 3 轮不动点）；修=`normalizeYuqueEmphasis` 入口掩码 `https?://` 段、出口还原（仅前端；后端镜像从无此拆分步骤）；(6) **彩色行内代码**（「颜色块/表格不支持行内代码」双 bug 共同根因，2026-09-01）——语雀把染色标签导出在**反引号内部**（`` `<font>preference</font>` ``、`_`/`**` 叠加、一对反引号内多段混排整行命令），旧 unwrap 剥反引号丢代码语义或 `normalizeLegacyHtmlTags` 无守卫改写反引号内标签致 code_inline 转义出字面 `<span>` 垃圾（表格管线本身无辜——管道表格转 HTML 前反引号已在兼容层被剥）；修=`convertBacktickedStyledCode`（前后端镜像）整段转原生 `<code>` 芯片**保码丢色**（`**`/`_`→strong/em 保留，残余活性字符转实体），**必须排在 `unwrapBacktickedEmphasis` 之前**，`normalizeLegacyHtmlTags` 加行内代码守卫；纯加粗 `` `**x**` `` 仍走旧 unwrap；(7) **相邻斜体粘连**（2026-09-01）——`_<font>A</font>__B_` 中间 `__` 按 CommonMark 侧翼规则（前标点后 CJK 字母）**只能开不能闭** → 配对全乱留字面 `_`；修=`normalizeItalicWrappingInlineHtml`（`normalizeBoldWrappingInlineHtml` 的 `_` 孪生，前后端镜像）`_(<font|span…>…)+_`→`<em>` 保色，A 消费 `__` 首 `_` 后裸残段自愈；正则要求 `_` 后紧跟标签，URL/snake_case 永不命中。**改 `applyYuqueCompatMode` 任何正则须过七类回归**：CJK 标点连接双加粗、font 交替染色句、含 ` --> ` 图表注释、空格包裹公式（抢救 vs 货币文本不触发）、URL 含 4+ 连续下划线/星号不被改写（掩码勿绕过）、反引号内彩标转 `<code>` 芯片不剥反引号（含表格单元格）、`_<font>…</font>_` 粘连斜体成对 `<em>` 无字面 `_`；排查导入渲染问题**先 dump 库里 `raw_content`**,勿用用户手头"同一篇"本地文件。详见 docs/editor.md §8。
- **Mermaid note 内列表 = 整图不渲染**（2026-09-01）：`stateDiagram` 的 `note … end note` 块内连续 `1.`/`-` 列表行被 marked 词法成**单个 list token**（文本保留内部 `\n`），note 宽到需折行时 `splitLineToFitWidth` 直接 throw 整图消失——Mermaid **上游 bug，11.17.2 仍未修**，触发条件「≥2 列表项+折行」故短列表从不炸、难缩小复现；Typora 正常是因其用默认 `htmlLabels:true`（HTML 排版折行，不走出错的 SVG 文本折行路径），简斋 `htmlLabels:false` 是 DOMPurify 剥 foreignObject 的安全加固**不可改回**。修=渲染时在标记与定界符间插 **U+2060 word joiner**（`1⁠.`）破坏列表识别、像素级无损：前端 `utils/mermaid.ts neutralizeNoteListMarkers`（`renderMermaid` 收口，三调用点全覆盖）+ 后端 `diagram_render.py` 镜像（`svg_map` 仍按**原始源码**做 key）；**勿改用 markdown 转义 `1\.`**——mermaid 的 markdown-to-lines 会把被转义定界符整个丢掉。存储不动、存量即生效。另：阅读页 mermaid 块数可远多于 `raw_content` 的 fence 数（`recoverYuqueDiagramComments` 渲染时从 HTML 注释恢复），排查时数量对不上先想这层。
- **PDF 阅读器滚动三坑（2026-09-01）**：① `PdfCanvas` 内**禁用 `scrollIntoView`**——它滚所有可滚祖先**含 window**，inner 模式（编辑器预览/附件面板/弹窗）会把整页滚走、工具条出视口；跳页/锚定一律走 `scrollPageIntoView`（inner 只滚容器、flow 滚窗口扣 sticky chrome），缩放/resize 用比例锚定（ratio 换算滚动位置）原地缩放勿跳页顶；② flow 工具条 sticky top 必须避开 `.blog-header`（sticky `z-index:30 height:60px` 会盖住 `top:0`；响应式变高故运行时测量勿硬编码）；③ **Playwright 滚动断言勿用 `locator.click()`**——对 sticky 元素 actionability 自动滚动走 CDP（JS hook 抓不到、按元素流内位置分步拖窗口，值可复现伪装成应用 bug），一律 `page.evaluate` DOM `el.click()`。
- **EPUB 阅读器八约定（2026-09-01，详见 docs/frontend.md §5 EPUB 阅读器）**：① **安全不靠 iframe sandbox**——章节在同源 `blob:` iframe 渲染，foliate 必须 `allow-scripts`（WebKit bug 218086），防线是后端上传即净化（`epub_sanitize.py`，干净文件字节原样存、脏文件重写 `mimetype` 首位 STORED）+ 前端 `transformTarget` 再剥 + 生产 Caddy CSP `script-src 'self' 'wasm-unsafe-eval'`（`blob:` 继承父页策略；改 CSP 须用 `vite preview` 带头重跑 Playwright，pdf.js 的 wasm 解码依赖 `'wasm-unsafe-eval'`）；② **注入章节的底色必须用预合成实底 `--jz-cell-surface`**（半透明 `--jz-surface` 会在 iframe 与分页背板叠两层出现色差；且勿把 `body` 列进「背景清透明」的结构元素规则——曾把自己设的底色抹掉），字体覆盖是**显式开关**（书自带字体默认尊重出版社），改用户表在 `utils/epubReader.ts buildEpubUserCss` 单源；③ **全屏用 Fullscreen API 作用于同一 wrapper、目录高亮用页顶标题语义 `pickActiveTocId`**——portal 搬移会让章节 iframe 重载丢分页状态；foliate 自带 `tocItem` 取「最后可见锚点」在细粒度目录下点 1.2 亮 1.3。④ **字体切换走语义映射不走一刀切**（`classifyFontFamily`/`buildFontMappingCss`：楷→文楷、黑→黑体、宋→所选正文、等宽→JetBrains Mono、其它不动；`pre/code` 永远等宽），出版社框/边框框 `data-jz-box/frame` 主题化重绘**勿再用「div 背景透明」一刀切删框**；**XHTML 章节 `tagName` 是小写**，DOM 判断一律 `localName`；拖动侧栏时 iframe 会吞 pointer 事件，须 `.is-resizing` 置 `pointer-events:none`。⑤ **装饰章节勿动 DOM**（CFI 是节点路径）：复制按钮用 `pre::after` + 热区点击、语法高亮用 CSS Custom Highlight API（`CSS.highlights` + `::highlight()`）、框/宽表用 data 属性；**剪贴板写入用章节 iframe 自己的 `navigator.clipboard`**（点击在 iframe 内，父文档未聚焦会被拒）。⑥ **翻页动画 = View Transitions**：过渡期 `html[data-jz-turn]{view-transition-name:none}` 让根退出快照，否则 theme.css 的 root 溶解会在每次翻页整页重放；所有 AntD 弹层 `getPopupContainer` 指向阅读器 wrapper，否则 **Fullscreen 下全部不可见**。⑦ **父页 `@font-face` 对章节 iframe 不可见**——自托管字体（宋体/思源黑体/文楷/等宽）在书里会静默退化成系统字体、切字体「没变化」；必须 `injectSiteFonts` 把父页相关 `@font-face` 规则（`url()` 解析为绝对地址）注入每个章节文档，排查字体问题用 CDP `CSS.getPlatformFontsForNode` 看实际渲染字体而非 `getComputedStyle`。⑧ **EPUB 的字体/字号/行距是 MD 阅读页同一份偏好**（`jz-article-font`/`jz-reader-font-scale`/`jz-reader-line-height`，基准 `--jz-fs-read`），EPUB 侧勿再新增私有字体/字号偏好；「效果一致」以 CDP 平台字体 + 计算字号/行高逐预设对比为准（`font_compare.py` 套路）。Playwright 在 closed shadow root 内的 iframe 里 `closest()` 找不到父层元素，须沿 `getRootNode().host` 上溯。vendor 补丁三处均在 `paginator.js`（`setStylesImportant` 空守卫 + 「自己向上跳」两处，见下条），更新 foliate 须重打（清单在 VENDOR.md）。窄屏 Playwright 点阅读器工具条会被 KB 抽屉 FAB 遮挡截胡，一律 `page.evaluate` DOM click。
- **MD 阅读页划线（2026-09-02，批次 A，详见 docs/frontend.md §5）**：锚=TextQuote（`utils/textAnchor.ts`：quote+prefix/suffix+标题 slug，重锚先精确后**空白归一化**回退，失效标「已失效」保留勿删）；绘制=**CSS Custom Highlight API**（`MdAnnotator`，零 DOM 注入；`::highlight()` 只认 color/background/text-decoration/text-shadow，无点击目标→点击走 `caretPositionFromPoint`+`isPointInRange`）；**锚定必须排除** `.jz-code-block`/链接卡/doc 卡/`.katex`/button/svg 子树（CardEnhancer 异步注水文本、LongImageEnhancer resize 包裹节点，全文偏移不稳）；`Highlight` 模型同表双锚（EPUB `cfi` / MD `selector` JSON 二选一，serializer 校验）；MD 回链 `/d/<id>?hl=<id>`（**同 slug 文档跨 KB 存在**，排查/冒烟勿硬编码 doc id，用 `resolve_public_post_by_slug` 取页面实际那篇）；读路径浮条与 SelectionAI **合并**（读态 `hideTrigger`+`externalRequest` 转发 AI 请求，编辑态原样；AI 按钮仅 staff）；新增 `explain` op（prompts.py + aiOps 两侧同步）。
- **EPUB 划线四坑（2026-09-02，详见 docs/frontend.md §5 二期 ①）**：① 划线走 foliate overlayer（`addAnnotation({value: cfi})` + `draw-annotation` 选 `Overlayer.highlight/underline`），**零 DOM 注入**；foliate 翻章即重建 overlayer，应用层持有列表在 `create-overlay` 回放；**view 未 `open` 时 `addAnnotation` 会抛**（列表先于书到达）→ 绘制/闪烁/跳转前守卫 `view.book`。② 选区在章节 iframe 内，父文档收不到 `selectionchange`——在 `load` 的 `doc` 上监听并经 `frameElement` 矩形映射到 stage 坐标。③ **章节加载后 `relocate` 会连发多次**（字体/ResizeObserver 重排）——`relocate` 里勿一刀切关闭浮层，卡片按 `resolveCFI().anchor(doc)` 重新锚定、浮条只在页首 CFI 变化时清除。④ 闪烁/深链的 overlay key 就是 CFI，与同 CFI 划线共用，删闪烁后须重绘该划线；划线章节名用 `getProgressOf(index, range)`（一 spine 节可含多个目录项）；Escape 处理须放在 `keyNav` 的 `isTypingTarget` 早退之前（焦点常在 Segmented `<input>`）。划线/书签**每用户私有**（作者也看不到读者的），AI 仍仅作者。
- **EPUB「自己向上跳」三坑（2026-09-02，详见 docs/frontend.md §5 / vendor VENDOR.md）**：① **全屏工具条只能纯覆盖，绝不能改 `.jz-epub-body` 的盒子**（曾用 padding-top 42↔0 带过渡——stage 逐帧变高触发 foliate ResizeObserver 3 次重排/8 次 relocate，正文整体位移；任何改 stage 尺寸的动画都会重演）；② foliate 视口无文字（封面、高于视口的截图）时 `getVisibleRange` 退化为 body 起点，锚点会把下次重排拉回章首——vendor 已补 `{element, delta}` 锚（滚动模式）/分数锚（翻页模式），更新 foliate 须重打；③ vendor `#justAnchored` 只能在真会发 scroll 事件时置位（`#scrollTo` 偏移不变早退路径传 `willScroll=false`），否则吞掉用户下一次滚动的锚点更新。排查同类「未翻页却跳」先在 `view.renderer` 上监听 `relocate`（带 `detail.reason`，view.js 转发会丢）并给 `.jz-epub-stage` 挂 ResizeObserver 计数；手动给 wrapper 加 class 会被 React 重渲染抹掉，要走真实全屏按钮。
...
- **字体层五约定（2026-08-10 / 09-01）**：① **font-family 一律走 tokens.css 的 `--jz-font-*` 令牌**（serif/sans/mono/display/kai/serif-en/decor/ui 八枚），勿再写散栈；`@font-face` 唯一入口 = `styles/fonts.css`（App.tsx 首位导入，全部 npm 自托管分片包，**index.html 严禁再引字体 CDN**——Google Fonts 国内不可达曾致设计字体全站从未生效）；② JS 侧栈唯一来源 `utils/fontStacks.ts`（与 tokens.css 逐字一致由 `fontStacks.test.ts` 锁定，改栈须两边同步）：DOM 内联/css-in-js 写 `'var(--jz-font-*)'`，canvas/可导出 SVG 用 `resolveFontVar()` 实值，**持久化进文档内容的**（Tiptap FONT_FAMILY_PRESETS）用实值常量勿用 var()；③ **display 令牌（霞鹜文楷 Screen）仅 400 字重**——消费点 font-weight 一律归一 400（伪加粗糊笔画、且与衬线兜底真 700 同行粗细不一），朱砂方印类留 serif；MiSans 是非标字重刻度（330/380/520/630），400/500 坍缩同档是固有特性勿修；④ 后台 MiSans 作用域靠 AdminLayout 挂 `<html data-scope='admin'>`（portal 弹层才解析得到），AntD `token.fontFamily` 是 `var(--jz-font-ui)` 透传，勿改回字面量栈；⑤ **排版阶梯与正文基类收口（2026-09-01 第二批）**：`--jz-fs-*`（3xs 10px–5xl 24px 十二档）/`--jz-lh-*`（四档）/`--jz-ls-*`（六档字距）已全站收编博客侧，新 CSS/TSX 字号行高字距**禁写裸值**优先令牌（26px+ 一次性展示字号、clamp 响应式标题、图标字形度量三类刻意豁免留字面值）；`.markdown-preview` 基类 = 正文排版**唯一收口**（`var(--jz-article-font, serif)` + `calc(--jz-fs-read × --jz-reader-scale)` + `var(--jz-reader-lh, --jz-lh-read)`，阅读三件套变量通道勿破坏），AI 面板经 `.jz-ai-md` 钉回 `--jz-font-ui`（工具 UI 非正文）、评论面板留 `--jz-fs-md` 侧栏密度；博客区标题衬线规则覆盖 `.blog-content h1-h6`（h4 卡片标题曾漏网致「页头宋体卡片黑体」）；emoji 栈第 9 枚令牌 `--jz-font-emoji`；`utils/typographyTokens.test.ts` 双守卫锁「readerLayout 行距三档 === tokens `--jz-lh-*`」与「styles/*.css 禁裸 font-family」。**导出端（headless Chromium）未装自托管字体，PDF 字形与网页不同为已知限制**（待镜像重建批次随包装入）。
...
- **线上**：腾讯云 `www.jianzhai.site`，线上 = main `3cba61e`（2026-09-02 晚纯前端快路径部署 EPUB「自己向上跳」修复：隔离缓存 build → rsync dist --delete → 只重建 caddy，入口 `index-C7Eg82al.js`、foliate 分片 `paginator-DIYuDj5w.js` 与本地一致、CSP/403/require_login 绿三件全过；同日早些标准路径部署 8a99bbc = 21319c8 之后 6 提交：Mermaid note 列表修复 / PDF 原地缩放 / 博客字体统一 / EPUB 一期 / 划线·笔记+KB 目录+EPUB 导航三批 / 下划线间距——dist + backend 双 rsync，从 `/root/jianzhai` 用 §3 钉死命令重建 backend/caddy + `up -d --no-deps backend celery caddy`；**新 app `apps.reading` 三迁移 0001–0003 已应用**，pyproject/Dockerfile/compose 未变；**服务器 `infra/Caddyfile` 已手工合并 CSP 头** `script-src 'self' 'wasm-unsafe-eval'`（EPUB 阅读器防线，备份 `Caddyfile.bak.20260902`），线上响应头实测带 CSP；绿三件（JS `index-CrOIIC7E.js` 一致 / 匿名 public 与 reading 端点 403 / `require_login:true`）+ 容器内 671 文档 5 用户仍在 `jianzhai_pg_data` 卷 + celery beat 启动全过。上一次 2026-09-01 部署语雀行内代码彩标+粘连斜体批次时无新迁移。**部署事故与修复**：服务器 `/root/jianzhai` 根目录另有一份只含 pg/redis 的 docker-compose.yml，在其中裸跑 `docker compose up -d` 曾致 compose v2 项目前缀把 postgres 切到全新空卷（`jianzhai_jianzhai_pg_data`），已当场指回原卷 `jianzhai_pg_data`（131MB 数据无损）并给该 compose 卷加显式 `name:` 钉死防再犯——**正确重建命令一律从 `/root/jianzhai` 跑 `docker compose -f infra/docker-compose.prod.yml --env-file infra/.env.prod build backend caddy && … up -d --no-deps backend celery caddy`**，详见 docs/deployment.md §3。**导出端 PDF 字体与网页不同**为已知限制，镜像未装自托管字体。2026-07-27 那次为**含镜像重建**部署：Playwright/Chromium 已进生产镜像（浏览器二进制走 npmmirror，服务器连不通官方 CDN），celery 已带 `-B` beat + `mem_limit 3g`（服务器 concurrency=1 变体保留）。**知识库内容未随 2026-07-27 同步完成**（当时在 media 阶段按用户要求中止，服务器为旧内容 176 篇 vs 本地 194 篇——07-27 快照；重跑 `Local_to_Cloud_Server_kb_sysnc.py` 可续传）。**语雀 URL 腐蚀修复已上线止损，但线上库存量坏 `raw_content` 不会自愈**（本地已重建 444/438，服务器侧需另行重建或随下次内容同步带过）。重部署两条路径与部署后验证绿清单 → `docs/deployment.md` §3
- **实现基线**（v0.9.10+，2026-08-04）：四角色 RBAC + 登录三因子 + 默认要求登录 + 受众可见性/ReadGrant 两道闸 · 6 主题 + 电影级主题过渡 · 三编辑器（Tiptap 3 / CM6 / HTML）+ 章节编号 + `[TOC]` · LaTeX/Mermaid 全链路（含导出端离线渲染）· Word/PPT 保真导入 + 语雀 MD 兼容 · 5 格式导出（结构化命名 / PDF+Word 三层目录体系：层级书签+卷首目录+篇内目录 / 站点 fail-closed / 保留清理与限流）· AI 多供应商 · PDF/PPT/**EPUB（foliate-js 原样阅读器：翻页/滚动 + View Transitions 翻页动画、CFI 续读、可拖宽折叠目录（约页码/筛选/字体颜色设置）、全书搜索、脚注弹卡、代码复制与 CSS Highlight 语法高亮、Readium 式主题注入与字体语义映射、字体/字号/行距与文章页共享、纸色、全屏沉浸；导入即净化 + 生产 CSP）**阅读器（**二期 ① 划线·笔记**：五色/下划线 overlayer 划线、笔记卡、侧栏笔记 tab、引用为 Markdown、导出读书笔记、`/d/<id>?cfi=` 深链，`apps.reading` 每用户私有；**MD 阅读页同款划线笔记**：TextQuote 锚 + CSS Highlight API + 目录/笔记右栏 + `?hl=` 深链，浮条并入 SelectionAI；**KB 目录 EPUB 范式**：左栏树两级展开/筛选/全展折/设置全套 + 知识库列表大类分组 + KB 页分组折叠，`treeToc/kbToc` 工具；**EPUB 导航批次 C**：搜索上一处/下一处+闪烁、书签 tab、章末「下一章」卡、读完页含同书架书目 `?doc_format=epub`）+ 阅读排版定制/专注模式/位置记忆 · 编辑器/预览样式体系优化（portal/冻结面/行内代码令牌，六主题 WCAG AA）· 动效体系（motion 令牌+`linear()` 真弹簧 / 路由连续性转场+共享元素 / 指针边缘光 / 收藏墨点粒子 / 动效三档位 / 导出实况胶囊 / 骨架屏 / skip-link）。**逐批次详情 → `docs/CHANGELOG.md`（时间序全量）；坑与决策复盘 → memory（`MEMORY.md` 索引）**
