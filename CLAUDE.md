...
- **字体层四约定（2026-08-10）**：① **font-family 一律走 tokens.css 的 `--jz-font-*` 令牌**（serif/sans/mono/display/kai/serif-en/decor/ui 八枚），勿再写散栈；`@font-face` 唯一入口 = `styles/fonts.css`（App.tsx 首位导入，全部 npm 自托管分片包，**index.html 严禁再引字体 CDN**——Google Fonts 国内不可达曾致设计字体全站从未生效）；② JS 侧栈唯一来源 `utils/fontStacks.ts`（与 tokens.css 逐字一致由 `fontStacks.test.ts` 锁定，改栈须两边同步）：DOM 内联/css-in-js 写 `'var(--jz-font-*)'`，canvas/可导出 SVG 用 `resolveFontVar()` 实值，**持久化进文档内容的**（Tiptap FONT_FAMILY_PRESETS）用实值常量勿用 var()；③ **display 令牌（霞鹜文楷 Screen）仅 400 字重**——消费点 font-weight 一律归一 400（伪加粗糊笔画、且与衬线兜底真 700 同行粗细不一），朱砂方印类留 serif；MiSans 是非标字重刻度（330/380/520/630），400/500 坍缩同档是固有特性勿修；④ 后台 MiSans 作用域靠 AdminLayout 挂 `<html data-scope='admin'>`（portal 弹层才解析得到），AntD `token.fontFamily` 是 `var(--jz-font-ui)` 透传，勿改回字面量栈。新增 font-size 优先 `--jz-fs-*` 阶梯。**导出端（headless Chromium）未装自托管字体，PDF 字形与网页不同为已知限制**（待镜像重建批次随包装入）。
