...
- **导出批次三坑（2026-07-27）**：① Chromium `page.pdf` 的 **`outline` 必须与 `tagged` 同开**——单独 `outline=True` 静默零书签（Chromium 147 实测）；② markdown-it 的 **`gfm_plugin` 已内置 tasklists**，再叠 `tasklists_plugin` 相互干扰致 checkbox 消失，勾选态在 `list_item_open.meta['checked']` 而非 html_inline；③ **静态站 fail-closed**：只收 `published_content` 非空文档、**绝不回落 raw**（与博客端 `resolve_published_html_body` 同规矩），改 `static_site` 勿复活回落。导出文件名唯一枢纽 = `common.build_export_filename`（`大类-知识库-[标题|N篇]-时间`），改命名规则勿散落 f-string；exporter 测试有 autouse fixture 隔离 MEDIA_ROOT（勿删，历史上曾污染真实 `exports/` 566 孤儿文件）。详见 docs/export-search.md。
...
- **线上**：腾讯云 `www.jianzhai.site`，线上 = main `31f687c`（2026-07-26 部署，无新迁移，最新迁移仍为 `accounts 0008`）。**main 已领先线上**：语雀公式抢救（81846d9）+ 导出大修批次（2026-07-27，无新迁移，但 `backend.Dockerfile` 新增 Playwright/Chromium——**部署须走含镜像重建的标准路径**，纯前端快路径不适用）。重部署两条路径与部署后验证绿清单 → `docs/deployment.md` §3
- **实现基线**（v0.9.10+，2026-07-27）：四角色 RBAC + 登录三因子 + 默认要求登录 + 受众可见性/ReadGrant 两道闸 · 6 主题 + 电影级主题过渡 · 三编辑器（Tiptap 3 / CM6 / HTML）+ 章节编号 + `[TOC]` · LaTeX/Mermaid 全链路（含导出端离线渲染）· Word/PPT 保真导入 + 语雀 MD 兼容 · 5 格式导出（结构化命名 / PDF 封面目录书签页码 / docx 真 Heading+TOC 域 / 站点 fail-closed / 保留清理与限流）· AI 多供应商 · PDF/PPT 阅读器 + 阅读排版定制/专注模式/位置记忆 · 编辑器/预览样式体系优化（portal/冻结面/行内代码令牌，六主题 WCAG AA）。**逐批次详情 → `docs/CHANGELOG.md`（时间序全量）；坑与决策复盘 → memory（`MEMORY.md` 索引）**
