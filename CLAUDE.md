...
- **实现基线**（v0.9.10+，2026-07-28）：四角色 RBAC + 登录三因子 + 默认要求登录 + 受众可见性/ReadGrant 两道闸 · 6 主题 + 电影级主题过渡 · 三编辑器（Tiptap 3 / CM6 / HTML）+ 章节编号 + `[TOC]` · LaTeX/Mermaid 全链路（含导出端离线渲染）· Word/PPT 保真导入 + 语雀 MD 兼容 · 5 格式导出（结构化命名 / PDF+Word 三层目录体系：层级书签+卷首目录+篇内目录 / 站点 fail-closed / 保留清理与限流）· AI 多供应商 · PDF/PPT 阅读器 + 阅读排版定制/专注模式/位置记忆 · 编辑器/预览样式体系优化（portal/冻结面/行内代码令牌，六主题 WCAG AA）。**逐批次详情 → `docs/CHANGELOG.md`（时间序全量）；坑与决策复盘 → memory（`MEMORY.md` 索引）**
